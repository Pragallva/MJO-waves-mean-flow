...
def integrate_model(input_file2):
    ...
    for ncycle in tqdm(range(itmax)):
        ...
        u_phi = ug*(phig + phi_B(Hmean))
        v_phi = vg*(phig + phi_B(Hmean))
    ...
    spatial_data  =  {'U': A(U),      'V': A(V),      'PHI': A(PHI),     'VRT': A(VRT),      'DIV': A(DIV),      \
                      ...
                      'phi_T': phi_T(lats, y0, H0,  N), 'phi_B': phi_B(Hmean)}
...
if __name__ == "__main__": 
    ...
    input_file ={   'nlons'   : 256,          \
                    ...
                    'path'    : '/data/pbarpanda/spherical_SWE/reproduce_monteiro_2014/narrow_forcing_phi_B_included/'} ; 
