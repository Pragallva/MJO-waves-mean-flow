...
def integrate_model(input_file2):
    ...
    for ncycle in tqdm(range(itmax)):
        ...
        ### Forcing term ###
        phi_forcing            = perturb(lons, lats, Q0, yp, xp, Lx, Ly)[1]
    ...
    path = '/data/pbarpanda/spherical_SWE/reproduce_monteiro_2014/narrow_forcing/'
    path = path +'/H0_%s/'%(Hmax)
    h5saveload.make_sure_path_exists(path)  
    h5saveload.save_dict_to_hdf5(spatial_data,  path+'spatial_data.hdf5')
    h5saveload.save_dict_to_hdf5(spectral_data, path+'spectral_data.hdf5')
    
    logging_object.write("Saved data in %s"%(path))
...
if __name__ == "__main__": 
    ...
    input_file['itmax']  = 100*int(86400/int(input_file['dt']))
