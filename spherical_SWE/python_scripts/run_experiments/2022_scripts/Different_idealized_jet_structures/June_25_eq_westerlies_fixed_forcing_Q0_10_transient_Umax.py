...
#       UU     = gauss_lat_curve( np.rad2deg(lats), mu,  sigma)*100   ##### Note zonal mean zonal U can also enter here as an array
        ...
        extra_gauss_term = (np.cos(np.deg2rad(np.rad2deg(lats)-mu))**10)*phimax/5
        extra_gauss_term1 = (np.cos(np.deg2rad(np.rad2deg(lats)-mu))**100)*phimax/5
        ...
        if int(t/(24*3600)) > 5 :
            if t % (4*24*3600) == 0: ### Save every 2 day   
                ...
                h5saveload.make_sure_path_exists('./temp_June26/')  
                h5saveload.save_dict_to_hdf5(temp_dict, './temp_June26/%d.hdf5'%(t/(24*3600)))
                ...
                h5saveload.make_sure_path_exists('./temp_June26/figures/') 
                py.savefig('./temp_June26/figures/U_%d_eq_easterly.png'%( int(t/(24*3600) )))
