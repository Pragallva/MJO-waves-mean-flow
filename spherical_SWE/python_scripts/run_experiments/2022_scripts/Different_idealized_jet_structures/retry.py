...
def integrate_model(input_file2):
    ...
    tmin_25       = 150*int(86400/dt) - tmax_25 - Q_spinup_time
    ...
    py.figure()
    py.plot(np.rad2deg(lats)[:,0], UU[:,0])
    py.savefig('./U_%d_%d.png'%(mu, sigma))
    
    phiT0 = extract_phiT_from_given_U(UU = UU, lat = np.rad2deg(lats), lon = np.rad2deg(lons))   

    for ncycle in tqdm(range(itmax), desc='mu = '+str(mu)):
...
#       UU     = gauss_lat_curve( np.rad2deg(lats), mu,  sigma)*100   ##### Note zonal mean zonal U can also enter here as an array
        ...
        if int(t/(24*3600)) > 5 :
            if t % (3*3600) == 0: ### Save every 3 hours        
                temp_dict = {'phiT':phiT, 'UU': UU, 'ug':ug, 'phig':phig, 'lat':np.rad2deg(lats), 'phiT0':phiT0, 'mu':mu, 'sigma':sigma}
                h5saveload.make_sure_path_exists('./temp/')  
                h5saveload.save_dict_to_hdf5(temp_dict,     './temp/%d.hdf5'%(t/(24*3600)))
...
#                 PHI_term3         .append(forcing_phi)       ## phi forcing
    ...
    spatial_data  =  {'U': A(U),      'V': A(V),      'PHI': A(PHI),  \
                      ...
                      'theoretical_Umean': UU[:,0], 'div': A(DIV),  'vrt': A(VRT),}
                    ...
                    for forcing_phase_speed in [0]: ##0                                            
                        for DIPOLE_or_MONOPOLE in [True]:
                            ...
                            for alphas, switch_on_days, keep_forcing_const_for_days  in zip([25], [0], [350]):
                                ...
                                for mu, sigma, den in zip([32], [10], [1.0]):   #### Not too sharp zonal wind but at different jet position
                                    ...
                                    input_file['U_up_days']      = (150 - input_file['alpha']*3)//2

                                    forcing_struct = 'dipole_heat' if DIPOLE_or_MONOPOLE else 'mono_heat'

                                    input_file['forcing_struct']   = forcing_struct
                                    input_file['path']             = \
                                    input_file['path']+'/dt_%d_Q_forcing_%d_forcing_y_%d_Hmean_%d_forcing_phase_speed_%d_ms/U_up_days_%d_mu_%d_sigma_%d/'%(\
                                                                                    input_file['dt'],\
                                                                                    input_file['Q0'],                  input_file['yp'], \
                                                                                    input_file['Hmean'], input_file['forcing_phase_speed'], \
                                                                                    input_file['U_up_days'], input_file['mu'], input_file['sigma'])
