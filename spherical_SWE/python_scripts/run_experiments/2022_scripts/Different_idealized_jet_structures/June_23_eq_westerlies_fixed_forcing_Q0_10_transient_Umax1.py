...
#     rest_time     = itmax - tmin_25 - tmax_25 - Q_spinup_time
    ...
    mu      = 36; sigma = 10
    UU1     = gauss_lat_curve( np.rad2deg(lats), mu,  sigma)*100*2   ##### Note zonal mean zonal U can also enter here as an array
    mu      = 0;  sigma = 10
    UU2     = gauss_lat_curve( np.rad2deg(lats), mu,  sigma)*100/1
    UU      = UU1  - UU2
    den     = 1
...
#       UU     = gauss_lat_curve( np.rad2deg(lats), mu,  sigma)*100   ##### Note zonal mean zonal U can also enter here as an array
        ...
        if int(t/(24*3600)) > 5 :
            if t % (4*24*3600) == 0: ### Save every 2 day   
                temp_dict = {'phiT':phiT, 'UU': UU, 'U': ug, 'lat':np.rad2deg(lats), 'phiT0':phiT0, 'phig_plus_phiT':phig_plus_phiT }
                        ...
                        for DIPOLE_or_MONOPOLE in [True]:
                            ...
                            for alphas, switch_on_days, keep_forcing_const_for_days  in zip([25], [0], [350]):
                                ...
                                input_file = {  'nlons'        : 256        , \
                                                ...
                                                'path'           : '/data/pbarpanda/spherical_SWE/evaluate_final_budget/transient_U_changing_jet_struct2/equator_easterlies/',} ; 
