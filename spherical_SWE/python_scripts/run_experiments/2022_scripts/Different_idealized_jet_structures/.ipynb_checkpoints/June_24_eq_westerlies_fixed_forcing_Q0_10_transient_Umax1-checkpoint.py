...
#     rest_time     = itmax - tmin_25 - tmax_25 - Q_spinup_time
    ...
    mu      = 36; sigma = 10
    UU1     = (np.cos(np.deg2rad(np.rad2deg(lats)-mu))**24 + \
               np.cos(np.deg2rad(np.rad2deg(lats)+mu))**24)*80   ##### Note zonal mean zonal U can also enter here as an array
    mu      = 0;  sigma = 10
    UU2     = (np.cos(np.deg2rad(np.rad2deg(lats)-mu))**4)*80
    UU3     = (np.cos(np.deg2rad(np.rad2deg(lats)-mu))**200)*500
    UU      = UU1  - UU2 + UU3*0
    den     = 1
    
    py.figure()
    py.plot(np.rad2deg(lats)[:,0], UU[:,0])
    py.savefig('./U_%d_%d_eq_easterly.png'%(mu, sigma))
    
    phiT0 = extract_phiT_from_given_U_try_new2(UU = UU, lat = np.rad2deg(lats), lon = np.rad2deg(lons))   
    ##extract_phiT_from_given_U(UU = UU, lat = np.rad2deg(lats), lon = np.rad2deg(lons))   
        ...
        if int(t/(24*3600)) > 5 :
            if t % (4*24*3600) == 0: ### Save every 2 day   
                temp_dict = {'lat':np.rad2deg(lats), 'lon':np.rad2deg(lons), 'phiT':phiT, \
                              'UU': UU, 'U': ug, 'lat':np.rad2deg(lats), 'phiT0':phiT0, 'phig_plus_phiT':phig_plus_phiT, 'phig':phig }
                h5saveload.make_sure_path_exists('./temp_June23/')  
                h5saveload.save_dict_to_hdf5(temp_dict,     './temp_June23/%d.hdf5'%(t/(24*3600)))
