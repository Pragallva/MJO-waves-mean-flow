...
if __name__ == "__main__": 
    ...
    for H0 in [0]:   #, 3000, 3500, 4000, 4500, 5000]:  
        for QQ0 in [10]:  ##tqdm([10], desc=str(H0)):  ### 0.1, 10, 50, 100, 125, 250, 500
            for HMEAN in [500]:  ##50   
                for forcing_WN in tqdm([1, 2, 3, 5, 8, 16, 20, 30, 40, 50]):  
                    for forcing_phase_speed in [5]:                                            
                        for DIPOLE_or_MONOPOLE in [True]:
                            for switch_off_day in [405]:
                                ...
                                logging_object = logruns.default_log(logfilename   = 'forcing_WN_%d'%(forcing_WN),  \
                                                                     log_directory = './log/')
                                ...
                                input_file = {  'nlons'        : 256        , \
                                                ...
                                                'y0'           : 0,  'N':2, 'Hmax': 2000, \
                                                'Hmean'        : HMEAN , \
                                                'Q0'           : forcing_WN*(QQ0)/2, 'yp'  : 0 , 'xp': 90 , 'Lx': 30 , 'Ly': 10, \
                                                'FUo'          : 0 ,  'FVo' : 0 ,  \
                                                'K_M'          : 20*24*3600, \
                                                'K_T'          : 10*24*3600, \
                                                'forcing_phase_speed': forcing_phase_speed, \
                                                'forcing_wave_number': forcing_WN , \
                                                'DIPOLE'             : DIPOLE_or_MONOPOLE, \
                                                'switch_off_day'     :  switch_off_day, \
                                                'path'               : '/data/pbarpanda/spherical_SWE/evaluate_final_budget/linear_Q_forcing/different_forcing_WN_same_energy/' } ; 

                                input_file['ntrunc']        = int(input_file['nlons']/3)
                                input_file['nlats']         = int(input_file['nlons']/2)
                                input_file['itmax']         = 257*int(86400/int(input_file['dt']))   #### Here 150 is in the units of days
                                input_file['Hmax']          = H0
                                ...
                                input_file['path']             = \
                                input_file['path']+'/dt_%d_Q_forcing_%d_forcing_y_%d_Hmean_%d_forcing_phase_speed_%d_ms_forcing_WN_%d/%s_switch_%s/'%(\
                                                                                input_file['dt']    ,\
                                                                                QQ0,   input_file['yp'], \
                                                                                input_file['Hmean'], input_file['forcing_phase_speed'], \
                                                                                input_file['forcing_wave_number'],\
