...
def PHI_perturb_propagating_separated_at_distance(lons, lats, Q0=100, yp=0, xp=90, Lx=30, Ly=10, time = 0, switch_on_day=None, c=5, wave_number=2, switch_off_day=None, separation_frac=0.5 ):
    ...
    separation_degrees = np.rad2deg(((2*np.pi)/wave_number)*separation_frac)      
    shift1  = kilometers2degrees((c*(time-switch_on_time))/1000)
    shift2  = kilometers2degrees((c*(time-switch_on_time))/1000) + separation_degrees
    
    xp1, yp1, Lx1, Ly1, shift1      = map(lambda z: np.deg2rad(z), [xp, yp, Lx, Ly, shift1 ])
    xp2, yp2, Lx2, Ly2, shift2      = map(lambda z: np.deg2rad(z), [xp, yp, Lx, Ly, shift2 ])
    
    heating_lon1 =   heating_monopole_lon(shift1, wave_number=wave_number)
    heating_lon2 =   heating_monopole_lon(shift2, wave_number=wave_number)
        
    phi_perturb1         =  flag * grav * Q0 *np.exp(  -(((lats-yp1)**2/Ly1**2)) ) * heating_lon1    
    phi_perturb2         = -flag * grav * Q0* np.exp(  -(((lats-yp1)**2/Ly2**2)) ) * heating_lon2
    
    phi_perturb          = phi_perturb1 + phi_perturb2
...
def integrate_model(input_file2):
    ...
    for ncycle in tqdm(range(itmax)):
        ...
        phi_forcing           = PHI_perturb_propagating_separated_at_distance(lons, lats, Q0, yp, xp, Lx, Ly, t, switch_on_day, \
                                                                              c=forcing_phase_speed, wave_number=forcing_wave_number, switch_off_day=switch_off_day,\
                                                                              separation_frac=separation_frac)[0]
...
if __name__ == "__main__": 
    ...
    for H0 in [3000,]:   #, 3000, 3500, 4000, 4500, 5000]:  
        for QQ0 in [10]:  ##tqdm([10], desc=str(H0)):  ### 0.1, 10, 50, 100, 125, 250, 500
            for HMEAN in [500]:  ##50   
                for separation_frac in tqdm([0.1, 0.5, 1, 1.5, 2, 2.5,]): #1, 1.5,   #2.5, 3, 3.5, 4, 6
                    for forcing_phase_speed in [5]:                                            
                        for DIPOLE_or_MONOPOLE in [False]:
                            for switch_off_day in [405]:
                                ...
                                logging_object = logruns.default_log(logfilename   = 'separation_frac_%1.1f'%(separation_frac),  \
                                                                     log_directory = './log/')
                                ...
                                input_file = {  'nlons'        : 256        , \
                                                ...
                                                'y0'           : 0,  'N':2, 'Hmax': 2000, \
                                                'Hmean'        : HMEAN , \
                                                'Q0'           : QQ0, 'yp'  : 0 , 'xp': 90 , 'Lx': 30 , 'Ly': 10, \
                                                'FUo'          : 0 ,  'FVo' : 0 ,  \
                                                'K_M'          : 20*24*3600, \
                                                'K_T'          : 10*24*3600, \
                                                'forcing_phase_speed': forcing_phase_speed, \
                                                'forcing_wave_number': 3 , \
                                                'DIPOLE'             : DIPOLE_or_MONOPOLE, \
                                                'switch_off_day'     : switch_off_day, \
                                                'separation_frac'    : separation_frac,\
                                                'sepration_degrees'  : np.rad2deg(((2*np.pi)/3)*separation_frac),\
                                                'path'               : '/data/pbarpanda/spherical_SWE/evaluate_final_budget/linear_Q_forcing/forcing_separated_by_distance/' } ; 
                                ...
                                input_file['path']             = \
                                input_file['path']+'/dt_%d_Q_forcing_%d_forcing_y_%d_Hmean_%d_forcing_phase_speed_%d_ms_forcing_separation_%1d/%s_switch_%s/'%(\
                                                                                input_file['dt']    ,\
                                                                                QQ0,   input_file['yp'], \
                                                                                input_file['Hmean'], input_file['forcing_phase_speed'], \
                                                                                input_file['sepration_degrees'],\
