...
def initial_conditions(sp_harmonic):
    ...
    ug   =   matsuno_mode['un']*fac
    vg   =   matsuno_mode['vn']*fac
    phig =   matsuno_mode['phin']*fac
...
#       phi_forcing           = (PHI_perturb_from_file(external_data, time = 0, switch_on_day=None)[0])
        ...
        if int(t/(24*3600)) > 5 :
            if t % (1*24*3600) == 0: ### Save every 1 day   
...
#                 h5saveload.make_sure_path_exists('./temp_June27/figures/') 
#                 py.savefig('./temp_June27/figures/U_%d_eq_easterly.png'%( int(t/(24*3600) )))
...
#     direc = './June13_22_Hmean_%d_ps_%d_Q0_%d/'%(Hmean,forcing_phase_speed, Q0)
#     h5saveload.make_sure_path_exists(direc)
                ...
                for forcing_phase_speed in [0]: ##0                                            
                    for DIPOLE_or_MONOPOLE in [True]:
                        ...
                        for alphas, switch_on_days, keep_forcing_const_for_days  in zip([100], [0], [350]):
                            ...
                            input_file['itmax']            = 2*800*int(86400/int(input_file['dt']))   #### Here 150 is in the units of days
                            input_file['Hmax']             = H0
                            input_file['switch_off_day']   = input_file['switch_on_day'] + input_file['keep_forcing_const_for_day']
                            input_file['yp']               = forcing_y_loc
                            input_file['U_up_days']        = 900 #(510 - input_file['alpha']*1)
                            ...
                            input_file['matsuno_mode']       = matsuno_modes['Kelvin']['(kn=-2)']
