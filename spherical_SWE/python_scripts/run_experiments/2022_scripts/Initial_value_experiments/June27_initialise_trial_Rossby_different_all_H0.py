...
#     H0_values[tmin_25 + tmax_25 + Q_spinup_time : ]                                   = 0
    ...
    for ncycle in tqdm(range(itmax)):
        ...
        if ncycle == tmax_25:
            ...
            h5saveload.make_sure_path_exists('./current_state_%s_previous/'%(current_time))  
            h5saveload.save_dict_to_hdf5(temp_dict, './current_state_%s_previous/%d.hdf5'%(current_time, t/(24*3600)))
            ...
            current_state = dict(ug=ug, vg=vg, phig=phig)            
            temp_dict  = current_state                
            h5saveload.make_sure_path_exists('./current_state_%s_after/'%(current_time))  
            h5saveload.save_dict_to_hdf5(temp_dict, './current_state_%s_after/%d.hdf5'%(current_time, t/(24*3600)))
...
#       phi_forcing           = (PHI_perturb_from_file(external_data, time = 0, switch_on_day=None)[0])
        ...
        if int(t/(24*3600)) > 1 :
            if t % (1*8*3600) == 0: ### Save every 3 hours 
                ...
                h5saveload.make_sure_path_exists('./temp_June27_init_try_%s/'%(current_time))  
                h5saveload.save_dict_to_hdf5(temp_dict, './temp_June27_init_try_%s/%d.hdf5'%(current_time, t/(24*3600)))
        ############################################################################################################################
