...
#   rest_time     = itmax   - tmin_25 - tmax_25 - Q_spinup_time
    ...
    for ncycle in tqdm(range(itmax)):
        ...
        mu = 0
        extra_gauss_term  = equator_wind_sign*np.nanmean(ug, axis=-1)[:,None]   #equator_wind*(np.cos(np.deg2rad(np.rad2deg(lats)-mu))**10)**2
                    ...
                    for DIPOLE_or_MONOPOLE in [True]:
                        ...
                        for alphas, switch_on_days, keep_forcing_const_for_days  in zip([25], [0], [350]):
                            ...
                            input_file = {  'nlons'          : 256        , \
                                            ...
                                            'equator_wind_sign' : +1, \
                                            'path'           : '/data/pbarpanda/spherical_SWE/evaluate_final_budget/Nudge_remove_U/' } ; 
