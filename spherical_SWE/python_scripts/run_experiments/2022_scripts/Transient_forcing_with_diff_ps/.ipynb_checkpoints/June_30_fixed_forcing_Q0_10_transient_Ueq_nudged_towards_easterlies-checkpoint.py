...
#   rest_time     = itmax   - tmin_25 - tmax_25 - Q_spinup_time
    ...
    for ncycle in tqdm(range(itmax)):
        ...
        mu = 0
        extra_gauss_term  = equator_wind*(np.cos( (lats-mu)**10) )**2
...
if __name__ == "__main__": 
    ...
    for Q0 in [10]:  ### 0.1, 10, 50, 100, 125, 250, 500
        for HMEAN, H0 in zip([200, 500], [2500, 5000]):  ##50   
            for forcing_y_loc in [0]:  
                for forcing_phase_speed in [0, 5, 15, 30, 40, 60, 80]: ##0                                            
                    for DIPOLE_or_MONOPOLE in [True]:
                        ...
                        for alphas, switch_on_days, keep_forcing_const_for_days  in zip([25], [0], [350]):
                            ...
                            input_file = {  'nlons'          : 256        , \
                                            ...
                                            'equator_wind'   : -10, \
                                            'path'           : '/data/pbarpanda/spherical_SWE/evaluate_final_budget/transient_U_propagate_forcing_diff_Heq_nudged_U/' } ; 
