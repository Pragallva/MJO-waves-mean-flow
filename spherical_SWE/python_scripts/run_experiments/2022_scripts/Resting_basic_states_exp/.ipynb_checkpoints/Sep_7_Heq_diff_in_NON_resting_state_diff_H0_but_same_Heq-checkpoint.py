...
if __name__ == "__main__": 
    ...
    def tropical_mask(lat, lon, field, bnd1=0, bnd2=10):    
    ...
    step  = 4
#     Umaxs = np.sort([int(i) for i in list(dicti['raw_field_dict'].keys())])[::step]
    Umaxs =  np.array([12, 10]) ### np.array([0, 14, 28, 42, 56, 70])
    H0_VALUES = np.sort([int(np.max(dicti['raw_field_dict'][str(key)]['phi_T']/grav)) for key in Umaxs])
    HEQs      = [500]
    for Umax in Umaxs:
        grav = 9.8 #m.s^2
        ds   = master_dict['500_0']['raw_field_dict'][str(Umax)]
        PHI_mean = ds['phi_B'] #latmean_near_equator(ds['lat'], ds['lon'], ds['PHI'])
        HEQs.append(PHI_mean/grav)
    
    for Q0 in [10]:  ### 0.1, 10, 50, 100, 125, 250, 500
        for H0, UMAX in zip(H0_VALUES, Umaxs):  ##50  
            for HMEAN in HEQs:
                for forcing_y_loc in [0]:  
                    for forcing_phase_speed in [0]: ##0                                            
                        for DIPOLE_or_MONOPOLE in [True]:

                            for alphas, switch_on_days, keep_forcing_const_for_days  in zip([25], [0], [1000]):
                                ...
                                input_file = {  'nlons'          : 256        , \
                                                ...
                                                'Umax'           : UMAX,\
                                                'H0'             : H0, \
                                                'path'           : '/data/pbarpanda/spherical_SWE/evaluate_final_budget/static_U_diff_Heq/' } ; 
                                ...
                                input_file['Hmax']             = H0
                                input_file['switch_off_day']   = input_file['switch_on_day'] + input_file['keep_forcing_const_for_day'] ## If switch_off_day> itmax, forcing will never switch off
                                input_file['yp']               = forcing_y_loc
                                input_file['U_up_days']        = input_file['itmax']/(int(86400/int(input_file['dt'])))

                                forcing_struct = 'dipole_heat' if DIPOLE_or_MONOPOLE else 'mono_heat'

                                input_file['forcing_struct']   = forcing_struct
                                input_file['path']             = \
                                input_file['path']+'/dt_%d_Q_forcing_%d_forcing_y_%d_Hmean_%1.1f_forcing_phase_speed_%d_ms/H0_%d_Umax_%d/'%(\
                                                                                input_file['dt'],\
                                                                                input_file['Q0'],                  input_file['yp'], \
                                                                                input_file['Hmean'], input_file['forcing_phase_speed'], \
                                                                                input_file['Hmax'], input_file['Umax'])


                                logging_object = logruns.default_log(logfilename   = 'Hmax_%d_Umax_%d_Sep7_Hmean_%1.1f_ps_%d_Q0_%d'%(\
                                                                                      input_file['Hmax'], input_file['Umax'], \
                                                                                      input_file['Hmean'], input_file['forcing_phase_speed'], \
                                                                                      input_file['Q0']),  \
                                                                     log_directory = './log/')
