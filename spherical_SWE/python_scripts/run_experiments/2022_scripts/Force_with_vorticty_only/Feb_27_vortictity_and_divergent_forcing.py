...
#         f_vrt_forcing_spec, f_div_forcing_spec    =   sp_harmonic.getvrtdivspec(fu_forcing, fv_forcing)
        ...
        flag = 1 if ((t >= switch_on_time) and (t <= switch_off_time))  else 0

        f_vrt_forcing_spec   =  flag*(sp_harmonic.grdtospec(momentum_forcing['vrt'])/K_M)
        f_div_forcing_spec   =  flag*(sp_harmonic.grdtospec(momentum_forcing['div'])/K_M)
...
if __name__ == "__main__": 
    ...
    for H0 in [3000, 1500, 0]:   #, 3000, 3500, 4000, 4500, 5000]:  
        for Q0 in tqdm([0], desc=str(H0)):  ### 0.1, 10, 50, 100, 125, 250, 500
            for HMEAN in [500]:  ##50   
                for forcing_y_loc in [0]:  
                    for forcing_phase_speed in [0]:                                            
                        for DIPOLE_or_MONOPOLE in [True]:
                            for switch_off_day in [405]:
                                ...
                                logging_object = logruns.default_log(logfilename   = 'H0_%d_Hmean_%d'%(H0, HMEAN),  \
                                                                     log_directory = './log/')
                                ...
                                input_file = {  'nlons'        : 256        , \
                                                ...
                                                'path'         : '/data/pbarpanda/spherical_SWE/evaluate_final_budget/momentum_forcing/',\
                                                'momentum_forcing_file': './momentum_forcing_data_from_H0_3000_Hmean_500_mu_40_sigma_8.hdf5' } ; 
                                ...
                                input_file['forcing_struct']   = 'vortcity_and_div_forcing_midlat_loc_40/'
                                input_file['path']             = \
                                input_file['path']+'/Hmean_%d_forcing_phase_speed_%d_ms/%s_switch_%s/'%(\
