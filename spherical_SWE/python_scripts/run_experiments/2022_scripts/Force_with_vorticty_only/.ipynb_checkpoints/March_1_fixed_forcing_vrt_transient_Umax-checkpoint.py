...
##########################################################################################################
        ...
        f_vrt_forcing_spec     =  (sp_harmonic.grdtospec(momentum_forcing['vrt']*momentum_forcing_scale)/K_M)*transient_effect 
        f_div_forcing_spec     =  (sp_harmonic.grdtospec(momentum_forcing['div']*momentum_forcing_scale)/K_M)*transient_effect*time_component  ##### No divergent forcing is applied now
...
if __name__ == "__main__": 
    ...
    for H0 in [5500]:   #, 3000, 3500, 4000, 4500, 5000]:  
        for Q0 in [0]:  ### 0.1, 10, 50, 100, 125, 250, 500
            for HMEAN in [500]:  ##50   
                for forcing_y_loc in [0]:  
                    for forcing_phase_speed in [5]: ##0                                            
                        ...
                        for alphas, switch_on_days, keep_forcing_const_for_days  in zip([25], [0], [350]):
                            ...
                            input_file['path']             = \
                            input_file['path']+'/Hmean_%d_forcing_phase_speed_%d_ms/%s/'%(\
