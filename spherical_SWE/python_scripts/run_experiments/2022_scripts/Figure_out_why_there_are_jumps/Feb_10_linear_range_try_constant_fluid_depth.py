...
#       phi_forcing           = (PHI_perturb_from_file(external_data, time = 0, switch_on_day=None)[0])
        ...
        if int(t/(24*3600)) > 200 :           
            if t % (3*3600) == 0: ### Save every 6 hours
                ...
                PHI_term1a        .append(-divg*(phi_B(Hmean)))
                PHI_term1b        .append(div_uvphi_NL_grid - (-divg*(phi_B(Hmean))))
                    ...
                    for forcing_phase_speed in [5]:                                            
                        for DIPOLE_or_MONOPOLE in [True]:
                            for switch_off_day in [405]:
                                ...
                                input_file['path']             = \
                                input_file['path']+'/dt_%d_Q_forcing_%d_forcing_y_%d_Hmean_%d_forcing_phase_speed_%d_ms_constant_fluid_depth/%s_switch_%s/'%(\
                                                                                input_file['dt']    ,\
