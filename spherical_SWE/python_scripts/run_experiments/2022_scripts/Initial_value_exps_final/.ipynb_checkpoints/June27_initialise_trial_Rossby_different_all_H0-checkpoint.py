...
def initial_conditions(sp_harmonic, wave_init=False, current_state = {'ug':0, 'vg':0, 'phig':0 }):
    ...
    if  wave_init:
        ...
        ug   =   current_state['ug']+matsuno_mode['un']*fac
        vg   =   current_state['vg']+matsuno_mode['vn']*fac
        phig =   current_state['phig']+matsuno_mode['phin']*fac
...
#   matsuno_modes = h5saveload.load_dict_from_hdf5('/data/pbarpanda/spherical_SWE/matsuno_modes.hdf5', track=False) 
    ...
    for Q0 in [0]:  ### Forcing is prescribed to be 0 here
        for HMEAN, H0 in zip([500], [100, 1000, 2000, 3000, 4000]):  ##50   
                ...
                for forcing_phase_speed in [0]: ##0                                            
                    for DIPOLE_or_MONOPOLE in [True]:
                        ...
                        for alphas, switch_on_days, keep_forcing_const_for_days  in zip([100], [0], [350]):
                            ...
                            input_file['path']             = \
                            input_file['path']+'/dt_%d_Q_forcing_%d_forcing_y_%d_Hmean_%d_forcing_phase_speed_%d_ms/U_up_days_%d_Rossby/'%(\
                                                                            input_file['dt'],\
                            ...
                            input_file['matsuno_mode']       = matsuno_modes['Rossby']['(n=1)']['(kn=-1)']
