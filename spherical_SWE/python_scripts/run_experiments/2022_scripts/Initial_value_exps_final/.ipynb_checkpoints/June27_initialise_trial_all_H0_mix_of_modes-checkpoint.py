...
import calc_matsuno_modes as mm
...
def initial_conditions(sp_harmonic, wave_init=False, current_state = {'ug':0, 'vg':0, 'phig':0 }):
            
    if  wave_init:
        fac  =   matsuno_amp_factor
        
        un = 0; vn=0; phin=0
        for m_mode in matsuno_mode:
            un   = un   + m_mode['un']
            vn   = vn   + m_mode['vn']
            phin = phin + m_mode['phin'] 
                    
        ug   =   current_state['ug']   + un*fac
        vg   =   current_state['vg']   + vn*fac
        phig =   current_state['phig'] + phin*fac
...
#     H0_values[tmin_25 + tmax_25 + Q_spinup_time : ]                                   = 0
    ...
    for ncycle in tqdm(range(itmax)):
        ...
        if ncycle == tmax_25:
            ...
            current_state = dict(ug=ug, vg=vg, phig=phig)            
            temp_dict  = current_state                
            h5saveload.make_sure_path_exists(path+'/current_state_%s_previous/'%(current_time))  
            h5saveload.save_dict_to_hdf5(temp_dict, path+'/current_state_%s_previous/%d.hdf5'%(current_time, t/(24*3600)))
            
            ug, vg, phig, vrtg, divg, vrtspec, divspec, KE_spec, phispec = initial_conditions(sp_harmonic, wave_init = True, current_state=current_state )
            
            
            current_state = dict(ug=ug, vg=vg, phig=phig)            
            temp_dict  = current_state                
            h5saveload.make_sure_path_exists(path+'/current_state_%s_after/'%(current_time))  
            h5saveload.save_dict_to_hdf5(temp_dict, path+'/current_state_%s_after/%d.hdf5'%(current_time, t/(24*3600)))
...
#         ######################################################################################
        ...
        phi_forcing           = ug*0 #PHI_perturb_propagating(lons, lats, Q0, yp, xp, Lx, Ly, t, switch_on_day, \
...
#       phi_forcing           = (PHI_perturb_from_file(external_data, time = 0, switch_on_day=None)[0])
        fu_forcing            = ug*0 #(F_uv_perturb(lons, lats, FUo, yp, xp, Lx, Ly, t, switch_on_day)[0])*time_component 
        fv_forcing            = vg*0 #(F_uv_perturb(lons, lats, FVo, yp, xp, Lx, Ly, t, switch_on_day)[0])*time_component 
...
#         if int(t/(24*3600)) > 1 :
#             if t % (1*8*3600) == 0: ### Save every 3 hours 
#                 temp_dict       =  {'U': A(U)[-1,...],      'V': A(V)[-1,...],      'PHI': A(PHI)[-1,...],  \
#                                    'lats': sp_harmonic.lats, 'lons': sp_harmonic.lons, 'T_in_days':A(T)[-1,...],  \
#                                    'phi_T': np.array(PHI_T)[-1,...], 'phi_B': phi_B(Hmean), 'phi_forcing': A(PHI_forcing)[-1,...], \
#                                    'vrt_forcing': A(F_vrt_forcing)[-1,...], 'div_forcing': A(F_div_forcing)[-1,...], 'abort_status':abort_status,\
#                                    'div': A(DIV)[-1,...], 'vrt': A(VRT)[-1,...]}
                
#                 h5saveload.make_sure_path_exists(path+'/temp_June27_init_try_%s/'%(current_time))  
#                 h5saveload.save_dict_to_hdf5(temp_dict, path+'/temp_June27_init_try_%s/%d.hdf5'%(current_time, t/(24*3600)))
#         ############################################################################################################################
...
#   matsuno_modes = h5saveload.load_dict_from_hdf5('/data/pbarpanda/spherical_SWE/matsuno_modes.hdf5', track=False) 
    HMEAN = 200
    for Q0 in [0]:  ### Forcing is prescribed to be 0 here
        for H0 in np.array([1000, 1500, 2000, 3000, 4000]):  ##50   
            for forcing_y_loc in [0]:  
                for forcing_phase_speed in [0]: ##0                                            
                    for DIPOLE_or_MONOPOLE in [True]:
                       ...
                       for alphas, switch_on_days, keep_forcing_const_for_days  in zip([100], [0], [350]):
                            ...
                            input_file = {  'nlons'          : 256        , \
                                            ...
                                            'FUo'            : 0 ,  'FVo' : 0 ,  \
                                            'K_M'            : 1e3*20*24*3600, 'K_T': 1e3*10*24*3600, \
                            ...
                            ###########################################################################
                            ################### To initialise with a given matsuno mode ###############
                            params = mm.params
                            params['Hmean']     = input_file['Hmean']
                            params['c']         = np.sqrt(input_file['grav']*input_file['Hmean'])
                            
                            params['yT0']       = np.sqrt(params['c']/params['beta'])
                            params['epsilon']   = 1.0
                            params['yT']        = np.sqrt(params['epsilon'])*params['yT0']
                            
                            matsuno_modes= mm.MODE_N(t = 0, params=params, grids=mm.grids)
                            h5saveload.save_dict_to_hdf5(matsuno_modes, './matsuno_modes_%d.hdf5'%(params['Hmean']))
                            
                            input_file['matsuno_mode1'] = matsuno_modes['Kelvin']['(kn=1)']
                            input_file['matsuno_mode2'] = matsuno_modes['Rossby']['(n=1)']['(kn=-1)']
                            input_file['matsuno_mode3'] = matsuno_modes['Kelvin']['(kn=2)']
                            input_file['matsuno_mode4'] = matsuno_modes['Rossby']['(n=1)']['(kn=-2)']
                            
                            input_file['matsuno_mode']       = [input_file['matsuno_mode1'], input_file['matsuno_mode2'], input_file['matsuno_mode3'], input_file['matsuno_mode4']] 
                            input_file['matsuno_amp_factor'] = 0.5
                            ############################################################################
                            ############################################################################
                            
                            logging_object = logruns.default_log(logfilename   = '%s_log_file'%(current_time),  \
                                                                 log_directory = './log/')
