...
def integrate_model(input_file2):
    ...
    for ncycle in tqdm(range(itmax), desc=str( int(K_M_DIV/(24*3600))) ):
...
#       phi_forcing           = (PHI_perturb_from_file(external_data, time = 0, switch_on_day=None)[0])
        ...
        damping_vrt                   = -vrtg/K_M_VRT ####### don't damp the vorticity
        ...
        damping_div                   = -divg/K_M_DIV  ####### only damp the divergence terms
                            ...
                            for DIPOLE_or_MONOPOLE in [True]:
                                for alphas, switch_on_days, keep_forcing_const_for_days  in zip([25], [0], [350]):
                                    ...
                                    input_file = {  'nlons'          : 256        , \
                                                    ...
                                                    'FUo'            : 0 ,  'FVo' : 0 ,  \
                                                    'K_M_DIV'        : 1e2*24*3600, 'K_M_VRT': K_M_VRT*24*3600, 'K_T': 10*24*3600, \
                                    ...
                                    input_file['path']             = \
                                    input_file['path']+'/Q_forcing_%d_forcing_y_%d_Hmean_%d_forcing_phase_speed_%d_ms/damp_div_only_%d_days_timescale/'%(\
                                                                                    input_file['Q0'],                  input_file['yp'], \
                                                                                    input_file['Hmean'], input_file['forcing_phase_speed'], \
                                                                                    input_file['K_M_VRT']/(24*3600))

                                    logging_object = logruns.default_log(logfilename   = 'Hmean_%d_'%(HMEAN, ),  \
                                                                         log_directory = './log/')
