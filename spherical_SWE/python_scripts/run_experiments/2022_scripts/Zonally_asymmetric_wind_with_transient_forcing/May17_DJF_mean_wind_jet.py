...
def phi_T_empirical(lon, lat, H0):
    ...
    phi_T = np.nanmean(phi_T, axis=-1, keepdims=True) #### Apply zonal mean jet
                ...
                for forcing_y_loc in [0]:  
                    for forcing_phase_speed in [0]: ##0                                            
                        for DIPOLE_or_MONOPOLE in [True]:
                            for xp in [50]: ### 0, 25, 50, 75, 100, 125
                                ...
                                input_file = {  'nlons'          : 256        , \
                                                ...
                                                'switch_on_day'  : 0, \
                                                'alpha'          : 15, \
                                                'keep_forcing_const_for_day' : 1000,\
                                                'path'           : '/data/pbarpanda/spherical_SWE/evaluate_final_budget/DJF_zonal_U_no_forcing/' } ; 
                                ...
                                input_file['switch_on_day']    =  input_file['U_up_days']     + input_file['alpha'] 
                                ...
                                #### Here 150 is in the units of days. This is just to make sure that the model is run for sufficient number of days after the model is spun up and after the heating is switched on.
