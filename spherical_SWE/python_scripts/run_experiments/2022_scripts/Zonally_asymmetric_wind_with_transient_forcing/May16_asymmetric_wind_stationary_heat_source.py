                        ...
                        for DIPOLE_or_MONOPOLE in [True]:
                            for xp in [0, 25, 50, 75, 100, 125]:
                                ...
                                input_file = {  'nlons'          : 256        , \
                                                ...
                                                'switch_on_day'  : 0, \
                                                'alpha'          : 15, \
                                                'keep_forcing_const_for_day' : 650,\
                                 ...
                                 #### keep_forcing_const_for_day = for how many days do you want to keep the forcing on? -- If you keep this longer than the total time integration, then the heating is never going to switch-off
                                ...
                                input_file['switch_on_day']    = input_file['U_up_days']     + input_file['alpha']//3                   #### This is to make sure that heating is switched on after the model is spun up after some time
                                input_file['switch_off_day']   = input_file['switch_on_day'] + input_file['keep_forcing_const_for_day'] #### If you keep this longer than the total time integration, then the heating is never going to switch-off
                                input_file['itmax']            = (input_file['switch_on_day']+300)*int(86400/int(input_file['dt']))     #### Here 150 is in the units of days. This is just to make sure that the model is run for sufficient number of days after the model is spun up and after the heating is switched on.
                                forcing_struct = 'dipole_heat' if DIPOLE_or_MONOPOLE else 'mono_heat'

                                input_file['forcing_struct']   = forcing_struct
                                input_file['path']             = \
                                input_file['path']+'/dt_%d_Q_forcing_%d_forcing_y_%d_forcing_x_%d_Hmean_%d_forcing_phase_speed_%d_ms_Uamp_factor_%d/'%(\
                                                                                input_file['dt'],\
