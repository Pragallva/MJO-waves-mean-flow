...
#         q0   = wave_amplitude*Dn(n = 0,  y=np.rad2deg(lats), yT=None, c = C,)
...
#         ###################################################
    ...
    if wave_init:
        ############## Rossby wave initialise ############
        C    = np.sqrt(grav*Hmean)
        q2   = wave_amplitude*Dn(n = 2,  y=np.rad2deg(lats), yT=None, c = C,)
        r0   = wave_amplitude*Dn(n = 0,  y=np.rad2deg(lats), yT=None, c = C,)/10
        v1   = wave_amplitude*Dn(n = 1,  y=np.rad2deg(lats), yT=None, c = C,)/20
    ...
    for ncycle in tqdm(range(itmax)):
        ...
        if int(t/(24*3600)) > 1 :
            ...
            if ((t % (8*3600) == 0) & (ncycle >= tmax_25)) :   
                ...
                dest = '/data/pbarpanda/spherical_SWE/initial_value_exps/Rossby_wave//H0_%d_Hmean_%d_figs'%(Hmax, Hmean)
...
if __name__ == "__main__": 
    ...
    for H0 in [0,]:   #, 3000, 3500, 4000, 4500, 5000]:  
        for HMEAN in [200]:  ##50   
            ...
            logging_object = logruns.default_log(logfilename   = 'H0_%d_Rossbt_wave_Hmean_%d'%(H0, HMEAN),  \
                                                 log_directory = './log/')
            ...
            input_file = {  'nlons'        : 256        , \
                            ...
                            'switch_off_day' :  0, \
                            'wave_amplitude' :0.1,\
                            'path'           : '/data/pbarpanda/spherical_SWE/initial_value_exps/Rossby_wave/' } ; 
