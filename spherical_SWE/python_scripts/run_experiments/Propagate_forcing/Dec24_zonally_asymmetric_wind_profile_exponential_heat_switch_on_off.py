# Dec12_zonally_asymmetric_wind_profile.py
...
# def phi_T_asymmetric(x, y, y0=0, H0=100,  N=2, \
#                      xN = 145,   yN = 45, \
#                      delta_xN = 35, delta_yN = 12, \
#                      xS = 145,   yS = 15, \
#                      delta_xS = 35, delta_yS = 15, \
#                      AN=0.45, AS=-0.25):
#     ## y takes latitude in radians
    
#     y0, xN, xS, yN, yS, delta_xN, delta_xS, delta_yN, delta_yS = \
#     map(lambda z: np.deg2rad(z), [y0, xN, xS, yN, yS, delta_xN, delta_xS, delta_yN, delta_yS])
    
#     phi1 = grav*H0*(1-(np.sin(y)-np.sin(y0))**N)
#     phi2 = grav*H0*(AN * np.exp(-((x-xN)/(delta_xN))**2 - ((y-yN)/(delta_yN))**2))
#     phi3 = grav*H0*(AS * np.exp(-((x-xS)/(delta_xS))**2 - ((y-yS)/(delta_yS))**2))
#     phi  = phi1 + phi2 + phi3    
#     return phi
...
# def phi_T_empirical(lon, lat):
#     path        = '/data/pbarpanda/spherical_SWE/empirical_data_reanalysis/'
#     phi_T_data  =  h5saveload.load_dict_from_hdf5(path+'phi_T_data.hdf5')
#     f     = interp2d(phi_T_data['lon'], phi_T_data['lat'], phi_T_data['phi_T_N'][0,:,:], kind='cubic')
#     phi_T = f(lon, lat[::-1])[::-1,:]   #### this is because
#     phi_T = (phi_T/np.max(phi_T) - np.min(phi_T/np.max(phi_T)))
#     return grav*H0*phi_T
...
def PHI_perturb_propagating(lons, lats, Q0=100, yp=0, xp=90, Lx=30, Ly=10, time = 0, switch_on_day=None, c=5, wave_number=2, DIPOLE = True, switch_off_day=None):
    ...
    spin_up_flag_time = 10
    flag_values = np.ones(itmax)*1
    tmax_25   = spin_up_flag_time*int(86400/dt)
    H0_spinup = np.linspace(0, 1, tmax_25)
    ...
    for ncycle in tqdm(range(itmax)):
        ...
        if int(t/(24*3600)) > 5 :
            ...
            if t % (3*3600) == 0: ### Save every 3 hours
                ...
                T.append(t/(24*3600)) 
                if (t/(24*3600)) == int((t/(24*3600))):
                    if ((int(t/(24*3600)))%10 == 0):
                        print (int(t/(24*3600)))
...
#         phiT = phi_T_asymmetric(x=lons, y=lats, y0=y0, H0=H0,  N=N, \
#                                                  xN = xN,   yN = yN, \
#                                                  delta_xN = delta_xN, delta_yN = delta_yN, \
#                                                  xS = xS,   yS = yS, \
#                                                  delta_xS = delta_xS, delta_yS = delta_yS, \
#                                                  AN=AN, AS=AS)
        
        phiT = phi_T_empirical( np.rad2deg(sp_harmonic.lons), np.rad2deg(sp_harmonic.lats), H0)
...
#         ##### Adding some additional terms #####
#         curl_uv_spec, div_uv_spec = sp_harmonic.getvrtdivspec(ug, vg)
#         div_uvphi_NL_spec     = div_uvphi_NL_spec + \
#                                 sp_harmonic.grdtospec(sp_harmonic.spectogrd(div_uv_spec)*(phi_B(Hmean) - phig))  
#         ########################################
...
#       phi_forcing           = (PHI_perturb_from_file(external_data, time = 0, switch_on_day=None)[0])
    ...
    spatial_data  =  {'U': A(U),      'V': A(V),      'PHI': A(PHI),     'VRT': A(VRT),      'DIV': A(DIV),      \
                      ...
                      'phi_T': phiT, 'phi_B': phi_B(Hmean), 'phi_forcing': A(PHI_forcing), \
    ...
    spectral_data =  {'U': A(U_spec), 'V': A(V_spec), 'PHI': A(PHI_spec),'VRT': A(VRT_spec), 'DIV': A(DIV_spec), \
                      ...
                      'phi_T': phiT, 'phi_B': phi_B(Hmean), 'phi_forcing':A(PHI_forcing_spec), \
                ...
                for forcing_phase_speed in [5]:                                            
                    for DIPOLE_or_MONOPOLE in [True]:
                        for switch_off_day in [405]:
                            ...
                            logging_object = logruns.default_log(logfilename   = 'empirical_phiT', \
                            ...
                            input_file = {  'nlons'        : 256        , \
                                            ...
                                            'xN'           : 145,  'yN': 10, 'delta_xN':30,  'delta_yN': 20, \
                                            'xS'           : 145,  'yS': -10,  'delta_xS':55,  'delta_yS': 5, \
                                            'AN'           : 0.45, 'AS': +0.0, \
                                            'FUo'          : 0 ,  'FVo' : 0 ,  \
                                            ...
                                            'path'               : '/data/pbarpanda/spherical_SWE/Hoskins_type_forcing/asymmetric_state_linear_forcing/' } ; 
                
                
                            input_file['ntrunc']        = int(input_file['nlons']/3)
                            input_file['nlats']         = int(input_file['nlons']/2)
                            input_file['itmax']         = 250*int(86400/int(input_file['dt']))   #### Here 150 is in the units of days
                            input_file['Hmax']          = H0
                            ...
                            input_file['path']             = \
                            input_file['path']+'/Q_forcing_%d_forcing_y_%d_Hmean_%d_forcing_phase_speed_%d_ms/%s_switch_%s/'%(\
