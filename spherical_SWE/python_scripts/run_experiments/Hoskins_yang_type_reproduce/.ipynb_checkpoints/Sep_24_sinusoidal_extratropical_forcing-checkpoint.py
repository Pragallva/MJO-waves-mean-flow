...
def integrate_model(input_file2):
    ...
    for ncycle in tqdm(range(itmax)):
        ...
        phi_forcing            = (perturb(lons, lats, Q0, yp, xp, Lx, Ly, t, switch_on_day)[1])*time_component 
    ...
    spatial_data  =  {'U': A(U),      'V': A(V),      'PHI': A(PHI),     'VRT': A(VRT),      'DIV': A(DIV),      \
                      ...
                      'phi_T': phi_T(lats, y0, H0,  N), 'phi_B': phi_B(Hmean), 'perturb': A(PHI_forcing) }
    spectral_data =  {'U': A(U_spec), 'V': A(V_spec), 'PHI': A(PHI_spec),'VRT': A(VRT_spec), 'DIV': A(DIV_spec), \
                      ...
                      'phi_T': phi_T(lats, y0, H0,  N), 'phi_B': phi_B(Hmean), 'perturb':A(PHI_forcing_spec)}
...
if __name__ == "__main__": 
    ...
    input_file['path']          = input_file['path']+'/phi_forcing/forcing_y_%d/time_period_%d_days/'%(forcing_y_loc, input_file['time_period'])
