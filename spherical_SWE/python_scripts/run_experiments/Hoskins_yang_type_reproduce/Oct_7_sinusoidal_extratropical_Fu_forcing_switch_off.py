...
def PHI_perturb(lons, lats, Q0=100, yp=0, xp=90, Lx=30, Ly=10, time = 0, switch_on_day=None, switch_off_day=None):
...
def F_uv_perturb(lons, lats, Fo=0.0001, yp=0, xp=90, Lx=30, Ly=10, time = 0, switch_on_day=None, switch_off_day=None):
...
def integrate_model(input_file2):
    ...
    for ncycle in tqdm(range(itmax)):
        ...
        ##### EXTERNAL FORCING ######
        time_component        = np.sin(2*np.pi*t/(time_period*24*3600))
        phi_forcing           = (PHI_perturb (lons, lats,  Q0, yp, xp, Lx, Ly, t, switch_on_day, switch_off_day)[0])*time_component 
        fu_forcing            = (F_uv_perturb(lons, lats, FUo, yp, xp, Lx, Ly, t, switch_on_day, switch_off_day)[0])*time_component 
        fv_forcing            = (F_uv_perturb(lons, lats, FVo, yp, xp, Lx, Ly, t, switch_on_day, switch_off_day)[0])*time_component 
    ...
    spatial_data  =  {'U': A(U),      'V': A(V),      'PHI': A(PHI),     'VRT': A(VRT),      'DIV': A(DIV),      \
                      ...
                      'vrt_forcing': A(F_vrt_forcing), 'div_forcing': A(F_div_forcing) }
    ...
    spectral_data =  {'U': A(U_spec), 'V': A(V_spec), 'PHI': A(PHI_spec),'VRT': A(VRT_spec), 'DIV': A(DIV_spec), \
                      ...
                      'vrt_forcing': A(F_vrt_forcing_spec), 'div_forcing': A(F_div_forcing_spec) }
...
# N=2
# H0s=(500)
# forc_locs=(4 8) ## (0 4 8 12 16 20) ##(10 20 30 40) 
# time_period=(2 5 10 15 20 25 30)

# for forc_loc in "${forc_locs[@]}"; do
# for H0 in "${H0s[@]}";do
# for T in "${time_period[@]}";do

    for H0 in [500, 2000]:
        for forcing_y_loc in [0, 4, 8, 12, 16, 20, 30, 40]:
            for time_period in [2, 5, 10, 15, 20, 25, 30]:
            
                start_time=ti.time() 

            #     H0            = int(sys.argv[1])
            #     forcing_y_loc = int(sys.argv[2])
            #     time_period   = int(sys.argv[3])
            
                switch_on_day  = 45
                FUo            = 0.0001 #float(sys.argv[4])  
                logging_object = logruns.default_log(logfilename   = 'H0=%d_force_y=%d_T=%d'%(H0, forcing_y_loc, time_period), \
                                                     log_directory = '~/Work/spherical_SWE/shell_scripts/log/')

                # forcing_y_%d/time_period_%d_days/'%(forcing_y_loc, input_file['time_period']

                input_file = {  'nlons'         : 256,          \
                                'ntrunc'        : int(256/3), \
                                'nlats'         : int(256/2), \
                                'dt'            : 150        , \
                                'itmax'         : 40*int(86400/150) , \
                                'spin_up_time'  : 25,     \
                                'rsphere'       : 6.37122e6  , \
                                'omega'         : 7.292e-5   , \
                                'grav'          : 9.80616    , \
                                ...
                                'y0'            : 0,  'N':2, 'Hmax': 2000, \
                                'Hmean'         : 800 , \
                                'Q0'            : 80 , 'yp'  : 0 , 'xp': 90 , 'Lx': 30 , 'Ly': 10, \
                                'FUo'           : FUo ,  'FVo' : 0 ,  \
                                'K_M'           : 20*24*3600, 'K_T': 10*24*3600, \
                                'switch_on_day' : 25,\
                                'switch_off_day': 90,\
                                'time_period'   : 2, \
                                'path'          : '/data/pbarpanda/spherical_SWE/Hoskins_type_forcing/' } ; 

                input_file['ntrunc']        = int(input_file['nlons']/3)
                input_file['nlats']         = int(input_file['nlons']/2)
                input_file['itmax']         = 200*int(86400/int(input_file['dt']))
                input_file['Hmax']          = H0
                input_file['switch_on_day'] = switch_on_day
                input_file['yp']            = forcing_y_loc
                input_file['time_period']   = time_period ## in days 
                input_file['path']          = \
                input_file['path']+'/Fu_forcing_switch_off/forcing_y_%d/time_period_%d_days/'%(forcing_y_loc, input_file['time_period'])
