...
def return_div_at_forcing(dicti, div_field, n = 0, lonn=None, latt=None):
...
def DIV(lon, lat, u, v, logging_object):
    R = 6371e3
    if logging_object is None:
        logging_object = logruns.default_log(logfilename   = 'momentum',  log_directory = './logs/')
    obj_momentum                  = momentum_advect.momentum_advection(u, v, lat, lon, logging_object)            
    dvdy  = obj_momentum.spher_div_y(v, lat,  lon, ax=-2)
    dudx  = obj_momentum.spher_div_x(u, lat,  lon, ax=-1)
    div   = (dudx+dvdy)
    return div

def plot_vectors(dir_name, movie_name,  dicti, dicti2, force_dict, tropical_lim=10, \
                 ...
                 SUPTITLE='', y=0, duration=300, loop=1, interval=2, forcing_scale=10**25, movie=True, fps=2, mag=None,\
                 lat_bnd = 0, lat_bnd1 = 85, l1=8, l2=2, FIGSIZE=(30,6)):
    
    if 'div' not in dicti.keys():
        div_field = DIV(np.rad2deg(dicti['lons']), np.rad2deg(dicti['lats']), dicti['U'],  dicti['V'], logging_object = None)
    else:
        div_field = dicti['div']
    ...
    for t in tqdm(all_time, desc = SUPTITLE): #tqdm
        ...
        FILENAME = dir_name+'/%s.png'%(str(t).zfill(7))
...
#             if mag is None:
            ...
            if eddy_lrange_style != 'Fixed':
                ...
                maxi   =  np.nanmax ([ np.abs(np.nanmin(eddy(div_field)[t])), np.abs(np.nanmax(eddy(div_field)[t])) ])
                mini   = -maxi
                NN     = 20
            ...
            field = ma.masked_where(((  np.abs(latt) < lat_bnd)  | (np.abs(latt) > lat_bnd1)), R*(div_field)[t, ...])
...
############################# Phi forcing ############################
            max_phi = np.max(np.abs(eddy(dicti['phi_forcing'][t, ...])))
            ax8.contour(np.rad2deg((dicti['lons'])),  np.rad2deg((dicti['lats'])),   eddy(dicti['phi_forcing'][t, ...]), \
                       [-(max_phi/1.1), (max_phi/1.1),],    colors='orange', linewidths=10, linestyles='solid'); 
            ax8.contour(np.rad2deg((dicti['lons'])),  np.rad2deg((dicti['lats'])),   eddy(dicti['phi_forcing'][t, ...]), \
                       [-(max_phi/1.1), (max_phi/1.1),],    colors='white', linewidths=3); 
############################# Phi forcing ############################            
    ...
    #             ax5.plot( np.array(time_so_far_ax5), -np.array(SUM_CONV)*R, '--', color='dodgerblue', lw=2)
            ...
            fig.suptitle(SUPTITLE, fontsize=20, y=1.2)
    ...
    if save_gif:
            ...
            movie_maker.make_movie(movie_name = movie_name, dir_name = dir_name, loop=0, movie=True, gif = False, fps=fps, duration = 300, interval=2 )        

if __name__ == "__main__":
    
    for extra in ['damp_div_by_1_days_and_vrt_by_50_days'] :#'alpha_5_switch_on_45_day', 'alpha_20_switch_on_80_day']:U_up_days_170 'U_up_days_237_mu_32_sigma_10', 'U_up_days_237_mu_55_sigma_10'
        for Hmean in ([500]):
            for H0 in ([5000]) : #3500 #3000, 1000, 0]): #0, 500, 1000, 1500, 2000, 2500, 3000, 3500 #0, 1000, 'alpha_20_switch_on_80_day'
                        
                    ps = 0;                                        
#                              /data/pbarpanda/spherical_SWE/evaluate_final_budget/play_damping//VRT_forcing_at_40_Hmean_500_forcing_phase_speed_0_ms/damp_div_by_1_days_and_vrt_by_50_days/H0_5000/
#                   source  = '/data/pbarpanda/spherical_SWE/evaluate_final_budget/play_damping//Q_forcing_10_forcing_y_0_Hmean_%d_forcing_phase_speed_%d_ms/%s/H0_%d/'%(Hmean, ps, extra, H0)

                    source     = '/data/pbarpanda/spherical_SWE/evaluate_final_budget/play_damping//VRT_forcing_at_40_Hmean_%d_forcing_phase_speed_%d_ms/%s/H0_%d/'%(Hmean, ps, extra, H0)

                    dicti_str  = 'H0_%d_Hmean_%d_%s'%( H0, Hmean, extra)
                    dir_name   = '/data/pbarpanda/spherical_SWE/Figures/%s%s/'%(dicti_str, '_midlat_VRT_forcing')
#                   dir_name   = '/data/pbarpanda/spherical_SWE/Figures/%s/'%(dicti_str)
                    movie_name = 'eddy_phi_divergence'
                    ...
                    if not os.path.exists(dir_name):                        
                        ...
                        if os.path.exists(source):
                            ...
                            plot_vectors(dir_name, movie_name, dicti = dicti, dicti2 = None, force_dict=force_dict, tropical_lim=15, \
                                         ...
                                         SUPTITLE=SUPTITLE, y=1.15, loop=0, movie=True, fps=10, mag=None, \
