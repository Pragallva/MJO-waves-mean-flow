...
#         all_time = [all_time[-1]]
    ...
    for t in tqdm(all_time, desc = SUPTITLE): #tqdm
        ...
        if not os.path.exists(FILENAME):
            ...
            max_phi = np.max(np.abs(eddy(force_dict['phi_forcing'][t, ...])))
            ax2.contour(np.rad2deg((dicti['lons'])),  np.rad2deg((dicti['lats'])),   eddy(force_dict['phi_forcing'][t, ...]), \
            ...
            ################### Inset figure #############################
            left, bottom, width, height = [0.8, 1.08, 0.2, 0.2]
            ax3 = ax2.inset_axes([left, bottom, width, height])
            F = np.copy(force_dict['phi_forcing'])   
            ...
            try:
                ...
                ax3.set_title('forcing scale', color='teal') 
            ...
            ################### Inset figure #############################
            left, bottom, width, height = [0.4, 1.08, 0.2, 0.2]
            
            R   = 6371e3
            
            sum_div_minus,   sum_div_plus   = return_at_forcing( dicti, force_dict, 'div',  n = t, lonn=lonn, latt=latt)
            ...
            try:
                ax5 = ax2.inset_axes([left, bottom, width, height])
    ...
    #             ax5.plot( np.array(time_so_far_ax5), -np.array(SUM_CONV)*R, '--', color='dodgerblue', lw=2)
            ...
            ################### Inset figure #############################
            left, bottom, width, height = [0.0, 1.08, 0.2, 0.2]
            ...
            ax2.set_title(variable+'\n Day %d'%(dicti['T_in_days'][t]), fontsize=20, y=1.4)
            ...
            ax2.quiverkey(Q, 0.3, 0.85, mag, r'%1.1e $m/s$'%(mag), labelpos='E', coordinates='figure', fontproperties = {'size':18})
...
if __name__ == "__main__":
    ...
    for extra in ['U_up_days_237_mu_45_sigma_10', 'U_up_days_237_mu_55_sigma_10'] :#'alpha_5_switch_on_45_day', 'alpha_20_switch_on_80_day']:U_up_days_170 'U_up_days_237_mu_32_sigma_10', 'U_up_days_237_mu_55_sigma_10'
        for Hmean in ([500]):
            for H0 in ([5000]) : #3500 #3000, 1000, 0]): #0, 500, 1000, 1500, 2000, 2500, 3000, 3500 #0, 1000, 'alpha_20_switch_on_80_day'
                for loc in [0]:
                    for Q0 in ([10]):       #50, 500         #10, 100 
                        ...
                        #dt_150_Q_forcing_10_forcing_y_0_Hmean_200_forcing_phase_speed_0_ms/U_up_days_217//H0_2500/
                        ...
                        #source = '/data/pbarpanda/spherical_SWE/evaluate_final_budget/transient_U_fixed_forcing/dt_150_Q_forcing_%d_forcing_y_%d_Hmean_%d_forcing_phase_speed_5_ms/%s/H0_%d/'%(Q0, loc, Hmean, extra, H0)
                        source  = '/data/pbarpanda/spherical_SWE/evaluate_final_budget/transient_U_changing_jet_struct2/dt_150_Q_forcing_%d_forcing_y_%d_Hmean_%d_forcing_phase_speed_%d_ms/%s/H0_%d/'%(Q0, loc, Hmean, ps, extra, H0)
                        #force_source = '/data/pbarpanda/spherical_SWE/evaluate_final_budget/transient_U_fixed_forcing/dt_150_Q_forcing_%d_forcing_y_%d_Hmean_%d_forcing_phase_speed_%d_ms/%s/H0_%d/'%(Q0, loc, Hmean, ps, 'U_up_days_217', H0)
                        
                        
                        dicti_str = 'Q0_%d_loc_%d_H0_%d_Hmean_%d_%s'%(Q0, loc, H0, Hmean, extra)
                        dir_name  = './Figures_different_fields_no_fixed_range/%s_phase_speed_%d_ms/eddy_phi'%(dicti_str, ps)
...
#                                 globals()['Q0_%d_loc_%d_H0_%d_Hmean_%d_%s_equation'%(Q0, loc, H0, Hmean, extra)] = h5saveload.load_dict_from_hdf5(source+'equation_data.hdf5')
                                ...
                                gif_name     = '/%s_phase_speed_%d_ms/eddy_phi'%(dicti_str, ps)



#                               globals()['force_dict'] =  h5saveload.load_dict_from_hdf5(force_source+'spatial_data.hdf5')
                                globals()['force_dict'] =  globals()['Q0_%d_loc_%d_H0_%d_Hmean_%d_%s'%(Q0, loc, H0, Hmean, extra)]
                                ...
                                plot_vectors(gif_name, dicti = dicti, dicti2 = None, force_dict=force_dict, \
                                             ...
                                             lat_bnd = 0, lat_bnd1 = 50, l1=14, l2=2, FIGSIZE=(15,8))
