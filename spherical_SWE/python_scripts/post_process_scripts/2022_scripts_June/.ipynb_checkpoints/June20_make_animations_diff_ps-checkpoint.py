...
if __name__ == "__main__":
    ...
    for Hmean in ([500]): ## 200
        for ps in ([0, 5, 15]) : #3500 #3000, 1000, 0]): #0, 500, 1000, 1500, 2000, 2500, 3000, 3500 #0, 1000, 'alpha_20_switch_on_80_day'
            for loc in [0]:
                for Q0 in ([10]):       #50, 500         #10, 100 

                    source  = '/data/pbarpanda/spherical_SWE/evaluate_final_budget/transient_U_propagate_forcing_diff_Heq/dt_150_Q_forcing_%d_forcing_y_%d_Hmean_%d_forcing_phase_speed_%d_ms/'%(Q0, loc, Hmean, ps)
                    source  = source + os.listdir(source)[0]+'/'
                    source  = source + os.listdir(source)[0]+'/'


                    dicti_str = 'Q0_%d_loc_%d_Hmean_%d'%(Q0, loc, Hmean)
                    dir_name  = './Figures_include_U_div_force/%s_phase_speed_%d_ms/eddy_phi'%(dicti_str, ps)

                    if not os.path.exists(dir_name):                        

                        if os.path.exists(source):
                            globals()['Q0_%d_loc_%d_Hmean_%d'%(Q0, loc, Hmean)] = h5saveload.load_dict_from_hdf5(source+'spatial_data.hdf5')
                            ...
                            print ('Q0_%d_loc_%d_Hmean_%d  does not exist yet'%(Q0, loc, Hmean))
