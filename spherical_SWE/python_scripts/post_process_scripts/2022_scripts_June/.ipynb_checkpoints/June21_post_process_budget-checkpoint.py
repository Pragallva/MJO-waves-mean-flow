...
def vorticity_budget(ds):
    ...
    VRT_dicti = {    'div'  : div,   'vrt': vrt,  'abs_vrt':abs_vrt,      \
                     ...
                     'damping_term':damping_term, 'dvrt_dt':ds['dvrt_dt'] }
...
def vorticity_budget_new(ds, ps):
    ...
    beta_term_div              =  -eddy(ds['V'])*d_by_dy(ds['lat'], ds['lon'], ( ZETA_a /cos_phi)[None,...])*cos_phi  
    beta_term_rot              =  -eddy(ds['V'])*d_by_dy(ds['lat'], ds['lon'], ( ZETA_a /cos_phi)[None,...])*cos_phi  
    ...
    VRT_dicti = {    'div'  : div,   'vrt': vrt,  'abs_vrt':abs_vrt,          \
                 ...
                 'V': eddy(ds['V']), 'cf':ps, 'cf_term':cf_term}
...
if __name__ == "__main__":
    ...
    source0  = '/data/pbarpanda/spherical_SWE/evaluate_final_budget/transient_U_propagate_forcing_diff_Heq/*'
    sources = glob.glob(source0)

    for source in sources:
        ps      = int(source.split('_')[-2])
